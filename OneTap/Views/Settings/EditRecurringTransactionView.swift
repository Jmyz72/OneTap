...
//  OneTap
//
//  View for editing existing recurring transactions
//
...
struct EditRecurringTransactionView: View {
    ...
    var body: some View {
        Group {
            if let viewModel {
                EditRecurringTransactionContent(viewModel: viewModel)
            } else {
                ProgressView()
            }
        }
        .onAppear {
            if viewModel == nil {
                viewModel = container.makeEditRecurringTransactionViewModel(recurring: recurring)
            }
        }
    }
}

private struct EditRecurringTransactionContent: View {
    @ObservedObject var viewModel: EditRecurringTransactionViewModel
